...
export const createArrayVisualizer = (config) => {
  const {
    title,
    pseudocodeLines,
    defaultArray = [9, 6, 4, 8, 2, 10, 1],
    generateSteps,
    complexity = { time: 'O(n)', space: 'O(1)' }
  } = config;

  return function ArrayVisualizer() {
    ...
    const getBgColor = (idx) => {
      if (step.result !== undefined && (step.result === idx || (Array.isArray(step.result) && step.result.includes(idx)))) return 'bg-green-600';
    ...
    return (
      <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
        ...
        {/* Two‑column layout: visualization left, pseudocode right */}
