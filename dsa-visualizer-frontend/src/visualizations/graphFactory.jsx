...
export const createGraphVisualizer = (config) => {
  const {
    title,
    pseudocodeLines,
    defaultGraph = { nodes: ['A', 'B', 'C', 'D', 'E'], edges: [['A', 'B'], ['A', 'C'], ['B', 'D'], ['C', 'E']] },
    generateSteps,
    complexity = { time: 'O(V+E)', space: 'O(V)' }
  } = config;

  return function GraphVisualizer() {
    ...
    const getNodeColor = (node) => {
      if (step.result === node) return '#22c55e';
      if (step.visited?.includes(node)) return '#6a9955';
    ...
    return (
      <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
        ...
        <div className="flex justify-center gap-4 mb-6">
          <div className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 text-gray-200">Graph is fixed for demo</div>
        </div>

        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3 space-y-6">
            <div className="flex justify-center mb-6">
              <svg width="500" height="400" viewBox="0 0 500 400" className="mx-auto">
                {graph.edges.map(([u, v], i) => (
                  <line key={i} x1={positions[u]?.x} y1={positions[u]?.y} x2={positions[v]?.x} y2={positions[v]?.y} stroke="#555" strokeWidth="2" />
                ))}
                {graph.nodes.map((node) => (
                  <g key={node}>
                    <circle cx={positions[node]?.x} cy={positions[node]?.y} r="24" fill={getNodeColor(node)} stroke="#222" strokeWidth="2" />
                    <text x={positions[node]?.x} y={positions[node]?.y + 5} textAnchor="middle" fill="white" fontSize="16" fontWeight="bold">{node}</text>
                  </g>
                ))}
              </svg>
            </div>
            ...
            <div className="flex justify-center gap-6 flex-wrap text-sm">
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-blue-500 rounded-full"></span>Unvisited</div>
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-yellow-500 rounded-full"></span>Current</div>
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-green-600 rounded-full"></span>Visited/Result</div>
            </div>
