...
const KokoBananas = () => {
  const pseudocodeLines = [
    "procedure minEatingSpeed(piles, H):",
  ...
  const defaultArrayInput = '3,6,7,11';
  const defaultTargetInput = '8';
  ...
  const generateSteps = (piles, H) => {
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Find minimum eating speed to finish [${piles.join(',')}] within ${H} hours`,
      pseudocodeLine: 1
    });
    ...
    while (low < high) {
      ...
      steps.push({
        ...
        mid,
        result: null,
        description: `mid = ${mid} → total hours = ${hours}`,
        pseudocodeLine: 3
      });
      ...
      if (hours <= H) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `hours ≤ H → feasible, try slower speed, high = ${high}`,
          pseudocodeLine: 6
        });
      } else {
        low = mid + 1;
        steps.push({
          ...
          mid,
          result: null,
          description: `hours > H → too slow, increase speed, low = ${low}`,
          pseudocodeLine: 7
        });
      }
      ...
      if (low < high) {
        steps.push({
          ...
          mid: null,
          result: null,
          description: `Now search speed in [${low}, ${high}]`,
          pseudocodeLine: 2
        });
      }
    }
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `Minimum eating speed = ${result}`,
      pseudocodeLine: 8
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Koko Eating Bananas</h2>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Piles (comma separated):</label>
        ...
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Hours H:</label>
