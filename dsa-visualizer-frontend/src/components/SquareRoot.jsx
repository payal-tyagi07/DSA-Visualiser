...
const SquareRoot = () => {
  ...
  const defaultArrayInput = '10'; // x
  const defaultTargetInput = ''; // not used
  ...
  const updateInputs = () => {
    const x = parseInt(arrayInput);
    if (isNaN(x) || x <= 0) return;
    // Create an array of indices 1..x for visualization
    const arr = Array.from({ length: x }, (_, i) => i + 1);
    setArray(arr);
  };

  const generateSteps = (arr) => {
    const x = arr.length; // the number
    const steps = [];
    let low = 1;
    let high = x;
    let ans = 0;
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Find integer square root of ${x}`,
      pseudocodeLine: 1
    });
    ...
    while (low <= high) {
      ...
      steps.push({
        ...
        mid,
        result: null,
        description: `mid = ${mid}, mid² = ${mid * mid}`,
        pseudocodeLine: 4
      });
      ...
      if (mid * mid === x) {
        ...
        steps.push({
          ...
          mid,
          result: mid,
          description: `mid² == ${x} → found exact root ${mid}! 🎉`,
          pseudocodeLine: 5
        });
        break;
      } else if (mid * mid < x) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `${mid}² = ${mid*mid} < ${x} → store ans = ${mid}, search right half, low = ${low}`,
          pseudocodeLine: 7
        });
      } else {
        high = mid - 1;
        steps.push({
          ...
          mid,
          result: null,
          description: `${mid}² = ${mid*mid} > ${x} → search left half, high = ${high}`,
          pseudocodeLine: 9
        });
      }
    ...
    if (!found) {
      steps.push({
        ...
        mid: null,
        result: ans,
        description: `Integer square root of ${x} is ${ans}`,
        pseudocodeLine: 10
      });
    }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Square Root (Integer)</h2>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Number x:</label>
          <input type="number" min="1" value={arrayInput} onChange={(e) => setArrayInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 focus:border-[#569cd6]" />
        </div>
