...
const KthMissing = () => {
  const pseudocodeLines = [
    "procedure findKthPositive(arr, k):",
    "  low = 0, high = len(arr)",
    "  while low < high:",
  ...
  const defaultArrayInput = '2,3,4,7,11';
  const defaultTargetInput = '5'; // k
  ...
  const generateSteps = (arr, k) => {
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Find ${k}th missing positive number in [${arr.join(',')}]`,
      pseudocodeLine: 1
    });
    ...
    while (low < high) {
      ...
      steps.push({
        ...
        mid,
        result: null,
        description: `mid = ${mid}, arr[${mid}] = ${arr[mid]}, missing before = ${missing}`,
        pseudocodeLine: 3
      });
      ...
      if (missing < k) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `missing < k → answer in right half, low = ${low}`,
          pseudocodeLine: 4
        });
      } else {
        high = mid;
        steps.push({
          ...
          mid,
          result: null,
          description: `missing ≥ k → answer in left half, high = ${high}`,
          pseudocodeLine: 5
        });
      }
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `${k}th missing positive number = ${result}`,
      pseudocodeLine: 6
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Kth Missing Positive Number</h2>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">k:</label>
