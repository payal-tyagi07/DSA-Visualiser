...
const LowerBoundSearch = () => {
  const pseudocodeLines = [
    "procedure searchInsert(arr, target):",
  "  low = 0",
  "  high = length(arr)",
  "  while low < high:",
  ...
  const [arrayInput, setArrayInput] = useState('1, 2, 4, 6, 8, 9, 10');
  const [targetInput, setTargetInput] = useState('5');
  const [array, setArray] = useState([1, 2, 4, 6, 8, 9, 10]);
  ...
  const generateSteps = (arr, t) => {
    ...
    while (low < high) {
      ...
      if (arr[mid] < t) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `${arr[mid]} < ${t} → low = mid + 1 = ${low}`,
          pseudocodeLine: 4
        });
      } else {
        high = mid;
        steps.push({
          ...
          mid,
          result: null,
          description: `${arr[mid]} >= ${t} → high = mid = ${high}`,
          pseudocodeLine: 6
        });
      }
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `Loop ended → lower bound at index ${result} (value = ${arr[result] !== undefined ? arr[result] : 'out of range'})`,
      pseudocodeLine: 7
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Lower Bound Search</h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <input type="text" value={arrayInput} onChange={(e) => setArrayInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 text-gray-200 focus:border-[#569cd6]" placeholder="e.g., 1,2,4,6,8,9,10" />
        </div>
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Target:</label>
          <input type="number" value={targetInput} onChange={(e) => setTargetInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 text-gray-200 focus:border-[#569cd6]" />
        </div>
        <button type="submit" className="bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-1 rounded transition-all">Update</button>
      </form>

      {!isSorted && <div className="bg-red-900/30 border border-red-500 text-red-300 p-3 rounded mb-4 text-center">⚠️ Array must be sorted.</div>}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3 space-y-6">
            ...
            <div className="flex justify-center gap-6 flex-wrap text-sm">
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-blue-500 rounded-full"></span><span>Not examined</span></div>
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-yellow-500 rounded-full"></span><span>Mid</span></div>
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-green-600 rounded-full"></span><span>Result</span></div>
              <div className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-[#4ec9b0]"></span><span>Low</span></div>
              <div className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-[#ce9178]"></span><span>High</span></div>
            </div>

            <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]">
              <p><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {steps[currentStep].description}</p>
            </div>
...
export default LowerBoundSearch;