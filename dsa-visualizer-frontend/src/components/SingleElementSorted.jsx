...
const SingleElementSorted = () => {
  const pseudocodeLines = [
    "procedure singleNonDuplicate(arr):",
  ...
  const [arrayInput, setArrayInput] = useState('1,1,2,3,3,4,4,8,8');
  const [array, setArray] = useState([1,1,2,3,3,4,4,8,8]);
  ...
  const generateSteps = (arr) => {
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Find single element in [${arr.join(', ')}]`,
      pseudocodeLine: 1
    });
    ...
    while (low < high) {
      ...
      if (mid % 2 === 1) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `mid is odd → decrement to ${mid}`,
          pseudocodeLine: 4
        });
      }
      ...
      if (arr[mid] === arr[mid + 1]) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `arr[${mid}] == arr[${mid+1}] → single element in right half, low = ${low}`,
          pseudocodeLine: 5
        });
      } else {
        high = mid;
        steps.push({
          ...
          mid,
          result: null,
          description: `arr[${mid}] != arr[${mid+1}] → single element in left half, high = ${high}`,
          pseudocodeLine: 7
        });
      }
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `Single element found at index ${result} (value = ${arr[result]}) 🎉`,
      pseudocodeLine: 8
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Single Element in Sorted Array</h2>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Array (sorted with pairs):</label>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3 space-y-6">
            ...
            <div className="flex justify-center gap-6 flex-wrap text-sm">
              ...
              <div className="flex items-center gap-2"><span className="w-4 h-4 bg-green-600 rounded-full"></span>Single</div>
...
export default SingleElementSorted;