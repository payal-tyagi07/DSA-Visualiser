...
const BookAllocation = () => {
  const pseudocodeLines = [
    "procedure allocateBooks(books, students):",
    "  if students > len(books): return -1",
    "  low = max(books), high = sum(books)",
    "  while low < high:",
    "    mid = floor((low + high) / 2)",
    "    if isPossible(books, students, mid): high = mid",
    "    else: low = mid + 1",
    "  return low",
    "",
  ...
  const defaultArrayInput = '12,34,67,90';
  const defaultTargetInput = '2'; // students
  ...
  const generateSteps = (books, students) => {
    if (students > books.length) {
      return [{
        ...
        mid: null,
        result: -1,
        description: `Not enough books for ${students} students.`,
        pseudocodeLine: 0
      }];
    }
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Allocate books [${books.join(',')}] to ${students} students, minimize max pages`,
      pseudocodeLine: 1
    });
    ...
    while (low < high) {
      ...
      steps.push({
        ...
        mid,
        result: null,
        description: `mid = ${mid} → needs ${count} students → ${possible ? 'feasible' : 'not feasible'}`,
        pseudocodeLine: 4
      });
      ...
      if (possible) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `Feasible → try smaller max, high = ${high}`,
          pseudocodeLine: 5
        });
      } else {
        low = mid + 1;
        steps.push({
          ...
          mid,
          result: null,
          description: `Not feasible → increase max, low = ${low}`,
          pseudocodeLine: 6
        });
      }
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `Minimum maximum pages = ${result}`,
      pseudocodeLine: 7
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Book Allocation</h2>
        <div className="flex justify-center gap-4 mb-4">
          <span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm border border-[#3c3c3c]">⏱️ Time: <span className="text-[#4ec9b0]">O(n log sum)</span></span>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Books pages (comma separated):</label>
        ...
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Students:</label>
          <input type="number" min="1" value={targetInput} onChange={(e) => setTargetInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 focus:border-[#569cd6]" />
        </div>
