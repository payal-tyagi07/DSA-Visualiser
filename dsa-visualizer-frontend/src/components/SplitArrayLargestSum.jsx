...
const SplitArrayLargestSum = () => {
  const pseudocodeLines = [
    "procedure splitArray(nums, m):",
  ...
  const defaultArrayInput = '7,2,5,10,8';
  const defaultTargetInput = '2'; // m splits
  ...
  const generateSteps = (nums, m) => {
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Split [${nums.join(',')}] into ${m} parts, minimize largest sum`,
      pseudocodeLine: 1
    });
    ...
    while (low < high) {
      ...
      steps.push({
        ...
        mid,
        result: null,
        description: `mid = ${mid} → need ${count} parts (max ${m}) → ${possible ? 'feasible' : 'too small'}`,
        pseudocodeLine: 3
      });
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `Minimum largest sum = ${result}`,
      pseudocodeLine: 6
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Split Array Largest Sum</h2>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Array:</label>
        ...
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Splits m:</label>
