...
const AggressiveCows = () => {
  const pseudocodeLines = [
    "procedure aggressiveCows(stalls, cows):",
    "  sort(stalls)",
    "  low = 1, high = stalls[-1] - stalls[0]",
    "  while low < high:",
    "    mid = floor((low + high + 1) / 2)",
    "    if canPlace(stalls, cows, mid): low = mid",
    "    else: high = mid - 1",
    "  return low",
    "",
  ...
  const defaultArrayInput = '1,2,8,4,9'; // unsorted stalls
  const defaultTargetInput = '3'; // cows
  ...
  const generateSteps = (stallsInput, cows) => {
    const stalls = [...stallsInput].sort((a, b) => a - b);
    const steps = [];
    let low = 1;
    ...
    steps.push({
      ...
      mid: null,
      result: null,
      description: `Sorted stalls: [${stalls.join(',')}], place ${cows} cows`,
      pseudocodeLine: 1
    });
    ...
    while (low < high) {
      ...
      steps.push({
        ...
        mid,
        result: null,
        description: `mid = ${mid} → can place ${count} cows (need ${cows}) → ${possible ? 'yes' : 'no'}`,
        pseudocodeLine: 5
      });
      ...
      if (possible) {
        ...
        steps.push({
          ...
          mid,
          result: null,
          description: `Possible → try larger distance, low = ${low}`,
          pseudocodeLine: 6
        });
      } else {
        high = mid - 1;
        steps.push({
          ...
          mid,
          result: null,
          description: `Not possible → try smaller distance, high = ${high}`,
          pseudocodeLine: 7
        });
      }
    ...
    steps.push({
      ...
      mid: null,
      result,
      description: `Largest minimum distance = ${result}`,
      pseudocodeLine: 8
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl transition-all duration-300 font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Aggressive Cows</h2>
        <div className="flex justify-center gap-4 mb-4">
          <span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm border border-[#3c3c3c]">⏱️ Time: <span className="text-[#4ec9b0]">O(n log range)</span></span>
      ...
      <form onSubmit={(e) => { e.preventDefault(); updateInputs(); }} className="flex flex-wrap justify-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Stall positions (comma separated):</label>
        ...
        <div className="flex items-center gap-2">
          <label className="text-[#9cdcfe]">Number of cows:</label>
          <input type="number" min="2" value={targetInput} onChange={(e) => setTargetInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 focus:border-[#569cd6]" />
        </div>
